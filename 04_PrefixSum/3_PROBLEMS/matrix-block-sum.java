...
public class matrix_block_sum {
    ...
    /**
     * Approach: 2D Prefix Sum Array To avoid summing the block repeatedly, we
     * precompute a 2D prefix sum array. Then for each cell, we define the
     * boundary of the block and use the 2D range sum formula.
     *
     * Time Complexity: O(M * N) for building prefix sum + O(M * N) for answers
     * = O(M * N) Space Complexity: O(M * N) for the prefix sum array
     */
