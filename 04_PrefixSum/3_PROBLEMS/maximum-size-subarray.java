 ...
 * Category: Medium (Premium problem, highly asked at Meta)
 * 
 * Problem: Given an integer array nums and an integer k, return the maximum length of a 
...
public class maximum_size_subarray {
     ...
     * We want to find max(i - j) such that PrefixSum[i] - PrefixSum[j] = k.
     * So we look for PrefixSum[j] = PrefixSum[i] - k in our map.
     * To maximize (i - j), we need to MINIMIZE j. Thus, we only store the EARLIEST index 
     * we see a particular prefix sum.
     * 
     * Time Complexity: O(N)
