 ...
 * Category: Medium (Greedy + Heap)
 * 
 * Problem: Reorganize characters so that no two adjacent characters are the same.
 */
public class reorganize_string {

    /**
     * Approach: Max-Heap (Frequency based)
     * Always pick the most frequent character that is different from the previous one.
     * 
    ...
    public String reorganizeString(String s) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (waitQueue.size() >= 2) {
                ...
                if (counts.get(front) > 0) pq.offer(front);
            }
        }
