...
public class circular_array_loop {
     ...
     * Each index i is a node, and the next node is (i + nums[i]) % n.
     * We run cycle detection for each starting index.
     * 
     * Time Complexity: O(N)
    ...
    private int getNext(int[] nums, int curr, boolean isForward) {
        ...
        if (isForward != currDir) return -1;
        
        int next = (curr + nums[curr]) % n;
        if (next < 0) next += n;
        
        if (next == curr) return -1; // Length must be > 1
        return next;
    }
