...
/**
 * LeetCode 1552. Magnetic Force Between Two Balls Category: Medium
 *
 * Problem: In the universe Earth C-137, Rick discovered a special form of
 * magnetic force between two balls if they are put in his new invented basket.
 * Rick has n empty baskets, the ith basket is at position[i], Morty has m balls
 * and needs to distribute the balls into the baskets such that the minimum
 * magnetic force between any two balls is maximum. Return the required
 * guaranteed maximum magnetic force.
 */
public class magnetic_force {

    /**
     * Approach: Binary Search on Answer This is EXACTLY the same problem as
     * "Aggressive Cows". Replace "baskets" with "stalls", and "balls" with
     * "cows". Magnetic force = Distance between two items. We want to Maximize
     * the Minimum distance.
     *
     * Time Complexity: O(N log(Max - Min) + N log N) Space Complexity: O(1)
     */
